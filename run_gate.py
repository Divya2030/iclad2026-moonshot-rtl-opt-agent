#!/usr/bin/env python3
# ---------------------------------------------------------------------------
# run_gate.py — Verification gate loop for the async_fifo RTL-optimization task.
#
# Wraps a candidate (agent-optimized) RTL set in three gates and emits a JSON
# verdict the agent can act on:
#
#   1. SIM   (mandatory, == the SCORED correctness check)  : SVUT + Icarus Verilog
#   2. EQUIV (strong guardrail, catches what the thin TB misses) : Yosys equiv
#   3. PPA   (only measured if SIM passes)                  : Yosys synth + OpenSTA
#
# Accept policy (tunable):  accept the rewrite ONLY if SIM passes. EQUIV is a
# strong signal but, for a CDC design, non-convergence != inequivalence, so we
# record it and let the agent decide how conservative to be. PPA numbers are a
# delta vs. the golden baseline so the agent can chase area/timing/power.
#
# This is a DRAFT harness: I have not executed it inside the eval Docker. The
# spots that depend on exact tool output formats are marked  # VERIFY:  — run
# each tool once and confirm the parser matches before trusting the numbers.
#
# Usage:
#   python3 run_gate.py --candidate <dir-with-changed-rtl> [--golden ../rtl]
#                       [--work .gate_work] [--dsize 32] [--asize 4]
#                       [--skip-ppa] [--json verdict.json]
#
# The candidate dir need only contain the files you changed; missing files are
# taken from --golden (overlay semantics).
# ---------------------------------------------------------------------------
import argparse
import json
import re
...
NVIDIA_ROOT = HERE.parents[1]                  # .../ICLAD26-NVIDIA-Problems
DESIGNS_DIR = HERE / "designs"

# Design-specific state, populated by configure() from a designs/<name>.json.
# Kept as module globals so the gate functions stay simple.
CFG = {}
DESIGN_ROOT = GOLDEN = SIM_DIR = SYN_DIR = None
RTL_FILES = []
TOP = None
EQUIV_DEFINES = []      # e.g. ["DSIZE=32", "ASIZE=4"]
SIM_SPEC = {}


def configure(config_path: Path):
    """Load a design config and set the module globals the gates rely on."""
...
def build_work_rtl(candidate: Path, golden: Path, work: Path) -> Path:
    """Overlay candidate files on top of golden into work/rtl."""
    rtl = work / "rtl"
    if rtl.exists():
        shutil.rmtree(rtl)
...
# --------------------------------------------------------------------------
# Gate 1: simulation (the scored correctness check) — mode dispatch
# --------------------------------------------------------------------------
...
def gate_sim_iverilog_diff(work_rtl: Path, work: Path):
    """Compile candidate RTL + TB(s) + harness fixture with iverilog, run vvp,
    and diff the extracted output against a golden expected file. Used for
    verilator-style pre-DV TBs (e.g. prim_crc32) that self-check via output."""
...
def gate_sim_svut(work_rtl: Path, work: Path):
    """SVUT unit-test suite (async_fifo), run once per -define set."""
...
def gate_sim_iverilog_tb(work_rtl: Path, work: Path):
    """Plain-Verilog testbenches (sha512): compile candidate RTL + each TB with
    iverilog, run vvp, parse the TB's own pass/fail tokens."""
    rtl = [str(work_rtl / f) for f in RTL_FILES]
...
def parse_svut(out: str, rc: int):
    # Validated vs real SVUT output. Two gotchas handled here:
    #  1. SVUT prints ANSI colour codes, e.g. "\x1b[1;31mERROR:" — the escape
    #     ends in 'm', so a \bERROR\b regex never matches. Strip ANSI first.
    #  2. svutRun returns exit 0 EVEN WHEN TESTS FAIL, so rc is not a gate.
    # Authoritative signal is SVUT's summary line: "- STATUS: X/Y test(s) passed".
    ...
    # Fallback if no summary (e.g. compile error): count failure markers.
...
def parse_iverilog_tb(name: str, out: str, rc: int):
    # Handles both secworks-style TB summaries:
    #   top  : "*** All NN test cases completed successfully."  /  "*** NN errors detected"
    #   core : "*** TC N successful."                            /  "*** TC N FAILED"
...
# --------------------------------------------------------------------------
# Gate 2: formal equivalence (Yosys-native)
# --------------------------------------------------------------------------
def gate_equiv(golden: Path, work_rtl: Path, work: Path):
    """Tiered equivalence:
      1. formal proof (Yosys, bounded by EQUIV_TIMEOUT) -> confidence 'proven'
      2. if formal is inconclusive/failed AND a sim-miter is configured, run it:
         miter PASS -> confidence 'sim-verified' (lower); miter FAIL -> reject.
    XOR/parity-heavy datapath rewrites are SAT-hard, so formal can time out on a
    correct design; the miter lets such high-value rewrites still be accepted,
    with the confidence level recorded explicitly.
    Returns proven(bool: formal only), verified(bool: formal OR sim), method,
    confidence."""
    eq = CFG.get("equiv", {})
    ...
    # formal did not prove — try the sim-miter fallback if configured
...
def gate_equiv_miter(golden: Path, work_rtl: Path, work: Path, spec: dict):
    """Compile golden and candidate top with the top module renamed apart, plus
    a miter fixture that drives both and prints MITER PASS/FAIL. Single-file
    designs only (multi-module designs would collide on submodule names — but
    those prove formally, so the fallback is not needed there)."""
    if len(RTL_FILES) != 1:
...
# --------------------------------------------------------------------------
# Gate 3: PPA (area / timing / power) via the repo's synth flow
# --------------------------------------------------------------------------
def gate_ppa(work_rtl: Path, work: Path):
    """Synthesize the candidate and read back area/timing/power. Two syn modes:
      run_syn_sh : async_fifo/sha512 — ./run_syn.sh all, VERILOG_FILES override.
      tcl        : OpenTitan prim — yosys -c syn.tcl + sta run_sta.tcl, driven by
                   a generated absolute-path VERILOG_FILELIST + DESIGN_NAME; reports
                   are <DESIGN>-prefixed.
    """
    import os
    syn = CFG.get("syn", {})
    ...
    if mode == "run_syn_sh":
        ...
        # candidate filelist with ABSOLUTE paths (filelist entries resolve
        # relative to the filelist's own dir, so absolute avoids surprises).
        fl = work / "candidate.f"
...
def parse_area(p: Path):
    # Validated vs Yosys stat: "Chip area for ... module ...: 122.6178".
    m = re.search(r"[Cc]hip area.*?:\s*([\d.]+)", _read(p))
    return float(m.group(1)) if m else None


def parse_wns(p: Path):
    # Validated vs repo sta_timing_report.txt summary row:
    #   "| WNS                  |          0.00 ps |              N/A |"
    # Setup WNS (column 1). 0.00 means no setup violation.
    m = re.search(r"\|\s*WNS\s*\|\s*(-?[\d.]+)\s*ps", _read(p))
    return float(m.group(1)) if m else None


def parse_worst_slack(p: Path):
    # Real timing margin: worst (minimum) "slack (MET|VIOLATED)" across all
    # paths in sta_timing_paths.txt. More informative than WNS for a design
    # that already meets timing (WNS clamps to 0).
...
def parse_bottleneck(p: Path):
    # Parse Yosys `stat -liberty` (synth_stat.txt): total/seq/comb area and the
    # top cell types by area. This tells the proposer where to aim — e.g. a
    # high seq_pct means "attack register count", a dominant XOR/adder class
    # means "attack that datapath operator".
    txt = _read(p)
    total = parse_area(p)
    ...
    # Two stat formats seen in the wild:
    #   A) `stat -liberty`: "<count> <area> <CellName_ASAP7>"  -> rank by area
    #   B) plain `stat`:    "<CellName_ASAP7> <count>"         -> rank by count
...
def parse_power(p: Path):
    # Validated vs repo sta_power_report.txt DESIGN POWER REPORT "Total" row:
    #   "Total   1.18e-03   1.24e-04   3.41e-07   1.30e-03  100.0%"
    #   columns: Internal  Switching  Leakage  >>Total<<  pct%
    m = re.search(
...
def main():
    ...
    ap.add_argument("--config", default=str(DESIGNS_DIR / "async_fifo.json"),
                    help="design config JSON (default: async_fifo)")
    ...
    # Gate 1 — SIM (scored correctness; mandatory)
    verdict["sim"] = gate_sim(work_rtl, work)

    # Gate 2 — EQUIV (strong guardrail)
    verdict["equiv"] = gate_equiv(golden, work_rtl, work)

    # Gate 3 — PPA (only if sim passed and not skipped)
    if verdict["sim"]["passed"] and not args.skip_ppa:
        verdict["ppa"] = gate_ppa(work_rtl, work)
    else:
        verdict["ppa"] = None

    # SIM is the hard gate. equiv_proven = formal proof; equiv_verified =
    # formal proof OR sim-miter fallback passed (lower confidence, recorded in
    # equiv.confidence).
    ...
    # Exit 0 when accepted AND equivalence is at least sim-verified. The
    # confidence level (proven vs sim-verified) is in the verdict for the
    # caller to weigh.
